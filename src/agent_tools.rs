...
dsh_web_pid() {{
  ...
  for proc in /proc/[0-9]*; do
    ...
    case "$cmd" in
      */data/data/com.termux/files/usr/bin/dsh\ web*|*/data/data/com.termux/files/usr/bin/dsh\ --profile\ web*|*/data/data/com.termux/files/usr/bin/dsh\ --profile=web*)
        printf '%s\n' "$pid";;
    esac
  done
}}
dsh_web_http_code() {{
  local code
  code=$(curl -s -o /dev/null -w '%{{http_code}}' -m 2 http://127.0.0.1:3080/ 2>/dev/null) || code=000
  printf '%s\n' "$code"
}}
dsh_web_url() {{
  local f url latest_url latest_mtime mtime
  latest_url=""
  latest_mtime=0
  for f in ~/dsh-web-restart.log ~/dsh-web-3080-current.log ~/dsh-web-3080.log; do
    [ -f "$f" ] || continue
    url=$(grep '^dsh web: http://127.0.0.1:3080/' "$f" 2>/dev/null | tail -1 | sed 's/^dsh web: //;s/[[:space:]]*$//')
    [ -n "$url" ] || continue
...
dsh_web_open_url() {{
  ...
  url=$(dsh_web_url)
  if [ -z "$url" ]; then
    echo "[dsh] 暂未找到带 token 的地址，看日志: grep '^dsh web:' ~/dsh-web-restart.log | tail -1"
    return 0
  fi
  echo "[dsh] 地址: $url"
  if command -v termux-open-url >/dev/null 2>&1; then
    (termux-open-url "$url" >/dev/null 2>&1 &)
    echo "[dsh] 已调用 termux-open-url 跳转浏览器"
  elif command -v termux-open >/dev/null 2>&1; then
    (termux-open "$url" >/dev/null 2>&1 &)
    echo "[dsh] 已调用 termux-open 跳转浏览器"
  elif command -v am >/dev/null 2>&1; then
    (am start -a android.intent.action.VIEW -d "$url" >/dev/null 2>&1 &)
    echo "[dsh] 已调用 am 跳转浏览器"
  elif command -v xdg-open >/dev/null 2>&1; then
    (xdg-open "$url" >/dev/null 2>&1 &)
    echo "[dsh] 已调用 xdg-open 跳转浏览器"
  else
    echo "[dsh] 未找到浏览器拉起命令，手动复制上面地址打开"
  fi
}}
dsh() {{
  if [ "$1" = "web" ]; then
    local web_pid code
    web_pid=$(dsh_web_pid | head -1)
    if [ -n "$web_pid" ]; then
      echo "[dsh] web 已在运行: $web_pid"
      code=$(dsh_web_http_code)
      case "$code" in
        [1-5][0-9][0-9]) echo "[dsh] http=$code";;
        *) echo "[dsh] 3080 未响应 (http=$code)";;
      esac
      dsh_web_open_url
      return 0
    fi
    ...
    for i in $(seq 1 30); do
      code=$(dsh_web_http_code)
      case "$code" in
        [1-5][0-9][0-9]) echo "[dsh] 就绪: http://127.0.0.1:3080 (http=$code)"; dsh_web_open_url; return 0;;
      esac
      sleep 1
    done
...
"###
    ...
    if let Some(start) = content.find(marker) {
        ...
        if content[after_marker..].contains("dsh_web_url()") {
            return Ok(());
        }
        // Replace any older managed block generated by this function (legacy
        // `dsh()` shape or intermediate `dsh_web_pid()` shape). If a user
        // reused the marker for unrelated shell content, leave it alone.
        let old_end = "\n  command dsh \"$@\"\n}\n";
        if let Some(relative_end) = content[after_marker..].find(old_end) {
            let end = after_marker + relative_end + old_end.len();
            let managed = &content[start..end];
            if managed.contains("dsh() {") && managed.contains("command dsh") {
                let replacement = wrapper.trim_start_matches('\n');
...
mod tests {
    ...
    fn dsh_web_wrapper_appends_to_bashrc() {
        ...
        assert!(
            content.contains("dsh_web_pid()") && content.contains("--profile"),
            "包装函数应识别 profile web 启动形式"
        );
        assert!(
            !content.contains("pgrep -f '[d]sh web'"),
            "包装函数不应只按 dsh web 文本匹配进程"
        );
        assert!(
            content.contains("dsh_web_url()") && content.contains("termux-open-url"),
            "包装函数应自动打印最新 token 链接并尝试拉起浏览器"
        );
    ...
    #[test]
    fn dsh_web_wrapper_replaces_legacy_managed_block() {
        let dir = tempfile::tempdir().unwrap();
        let home = dir.path();
        let marker = "# dsh web: 自动脱离终端后台启动";
        let legacy = format!(
            "prefix\n{marker}\ndsh() {{\n  if [ \"$1\" = \"web\" ]; then\n    setsid nohup /data/data/com.termux/files/usr/bin/dsh web &\n  fi\n  command dsh \"$@\"\n}}\nuser-content\n"
        );
        fs::write(home.join(".bashrc"), legacy).unwrap();

        patch_dsh_web_wrapper(home).unwrap();

        let content = fs::read_to_string(home.join(".bashrc")).unwrap();
        assert!(
            content.contains("dsh_web_pid()"),
            "应写入新的 profile 检测器"
        );
        assert!(
    ...
    #[test]
    fn dsh_web_wrapper_upgrades_pid_only_block_to_url_opener() {
        let dir = tempfile::tempdir().unwrap();
        let home = dir.path();
        let marker = "# dsh web: 自动脱离终端后台启动";
        ...
        fs::write(home.join(".bashrc"), intermediate).unwrap();
        ...
        assert_eq!(content.matches(marker).count(), 1, "升级时不应重复写入标记");
    }
