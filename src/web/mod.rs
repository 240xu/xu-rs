...
fn route(home: &Path, request: &Request, expected_csrf: &str) -> RouteResponse {
    match (request.method.as_str(), request.path.as_str()) {
        ...
        ("GET", "/api/overview") if host_is_local(request) => json_route(api::overview(home)),
        ("GET", "/api/providers") if host_is_local(request) => json_route(api::providers(home)),
        ...
        ("GET", "/api/mcp") if host_is_local(request) => json_route(api::mcp_list(home)),
        ("GET", "/api/skills") if host_is_local(request) => json_route(api::skills_list(home)),
        ("GET", "/api/stats") if host_is_local(request) => json_route(api::stats(home)),
        ("GET", "/api/sessions") if host_is_local(request) => json_route(api::sessions()),
