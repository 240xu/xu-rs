...
fn help_text() -> String {
    "trivium — terminal provider switcher\n\n  trivium | xcc | spec         open TUI\n  trivium list                    list providers\n  trivium agent doctor            诊断 Termux 客户端环境\n  trivium agent setup             预览 OpenCode/Claude/Codex/Dsh 安装更新\n  trivium agent setup --yes       执行 OpenCode/Claude/Codex/Dsh 安装更新\n  trivium agent status [--latest] 显示客户端版本与状态\n  trivium agent install <id|all>  预览安装/更新（显示 旧版→新版）\n  trivium agent install <id|all> --yes 执行安装/更新与重装检测\n  trivium provider preset list    list provider templates\n  trivium provider add <id> ...   add provider\n  trivium provider add <id> --preset openrouter --api-key <key>\n  trivium provider models <id>    fetch models from /models\n  trivium provider models <id> --apply --yes update stored models\n  trivium provider update <id> ... update provider\n  trivium provider duplicate <source> <new-id> copy provider\n  trivium provider show <id>      show provider without secrets\n  trivium provider delete <id> --yes delete provider\n  trivium mcp list                list unified MCP servers\n  trivium mcp add <id> ...        preview MCP add and projections\n  trivium mcp add <id> ... --yes  add and project MCP server\n  trivium mcp import --target <app> [--yes] import live MCP definitions\n  trivium mcp enable <id> --target <app> [--yes]\n  trivium mcp disable <id> --target <app> [--yes]\n  trivium mcp apply [--target <app|all>] [--yes]\n  trivium mcp delete <id> [--yes] remove from SSOT and live configs
  spec sync mcp <source> <target...> [--dry-run] sync MCP servers across agents\n  trivium current [target]        show recorded current provider\n  trivium use <id> [--target t]   apply provider to target\n  trivium use <id> --dry-run      preview writes\n  trivium test <id>               test provider connectivity\n  trivium doctor                  show paths and current state\n  trivium config path             show managed config paths\n  trivium sync skills <src> <dst...> [--dry-run]  mirror skill dirs across agents\n  trivium serve                   run local protocol adapter\n  trivium web [--port N]            run local web console\n"
        .to_string()
}
